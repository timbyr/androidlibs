...
public class AutoResizeTextView extends TextView {
    ...
    public void resizeText(int width, int height) {
        ...
        int textHeight = getTextHeight(text, paint, width, targetTextSize);
        int textWidth = getTextWidth(text, paint, width, targetTextSize);
        ...
        while (((textHeight > height) || (textWidth > width))
            ...
            textWidth = getTextWidth(text, paint, width, targetTextSize);
        }
        ...
        if (mAddEllipsis && targetTextSize == mMinTextSize && textHeight > height) {
            ...
            while (width < lineWidth + ellipseWidth) {
                lineWidth = paint.measureText(text.subSequence(start, --end + 1)
                        .toString());
            }
    ...
    // Set the text size of the text paint object and use a static layout to
    // render text off screen before measuring
    private int getTextWidth(CharSequence source, TextPaint originalPaint, int width,
            float textSize) {
        // Update the text paint object
    	TextPaint paint = new TextPaint(originalPaint);
        paint.setTextSize(textSize);
        // Draw using a static layout
        StaticLayout layout = new StaticLayout(source, paint, 4096,
                Alignment.ALIGN_NORMAL, mSpacingMult, mSpacingAdd, true);
        layout.draw(sTextResizeCanvas);
